...
- **Escopo de campus, no RERANK (não na query)**: a base é toda do Campus Canoas, mas contém docs INSTITUCIONAIS do IFRS (multi-campus, ex: o "PDI IFRS 2024-2028") que dominam a similaridade e enterram a resposta certa (a query "salas dos professores prédio" trazia 28/30 chunks do PDI). Duas peças: (a) `FETCH_K=60` (pool grande o bastante para o doc certo de Canoas, que fica além do top-30 quando o institucional domina a similaridade crua, entrar no pool); (b) penalidade no rerank `ALPHA*sim + (1-ALPHA)*date_score - CAMPUS_PENALTY`, com `CAMPUS_PENALTY=0.35` para chunks `campus_scope=="outro"` (ausência = neutro 0), valor calibrado para tirar o PDI do contexto sem margem apertada. O escopo age no rerank, não na query, pois ancorar a query ao campus (`f"{query} IFRS Campus Canoas"`) infla editais institucionais e enterra a resposta em queries de professor. O `campus_scope` é atribuído no ingest por `classify_campus_scope` (ALTA PRECISÃO, para não expulsar doc de Canoas): tagueia "outro" só (a) fora do `/canoas/` E (b) com marcador institucional explícito (título "PDI"/"Plano de Desenvolvimento Institucional" ou >=4 campi do IFRS citados). NUNCA tagueia doc sob `/canoas/` (Canoas-específico mesmo citando a rede - senão o relatório CPA de Canoas virava "outro"). Hoje "outro" = o PDI (Drive, 269 chunks) + planos de ação IFRS institucionais (no `/site/`, não `/canoas/`). Além da penalidade, um CAP DURO `CAMPUS_OUTRO_MAX=0` (em `build_context`) EXPULSA do contexto todo chunk `campus_scope=="outro"`: a penalidade só REBAIXAVA o institucional, que ainda sobrava no top-15 e vazava (ex: "Torre Norte" do PDI aparecia nas salas dos professores); com o cap, nenhum institucional entra no contexto (a base é toda de Canoas, o institucional não responde daqui). Limite conhecido: uma pergunta legitimamente institucional (reitor/PDI) poderia ficar sem contexto; se reaparecer, liberar alguns slots "outro" só quando o contexto ficar vazio. **Refino POR CHUNK (`refinar_campus_scope`, no `chunk_document`)**: o doc-nível é grosso demais, pois o PDI tem seções ESPECÍFICAS de Canoas (o Quadro 5.3 de vagas, a seção 5.1.3, a infraestrutura 6.1.3) que o cap expulsava junto com o resto. Num doc "outro", um state machine determinístico (zero LLM) acompanha a "seção em vigor" pelos cabeçalhos (T1 legenda "Quadro N ... Campus X"; T2 heading numerado "5.1.3. Canoas"; T3 linha isolada "Campus X") e libera do "outro" só os chunks que tocam uma seção de Canoas. Anti-falso-positivo (calibrado no PDI real): linha de sumário não transiciona (leader de pontos na linha ou na SEGUINTE, entrada quebrada), célula de tabela não transiciona (T3 suprimido com vizinho percentual "N%"; nome de campus SEM o prefixo "Campus" nunca casa, o que já exclui as tabelas-resumo por campus), e heading numerado todo em maiúsculas volta a seção para "rede". Alternativas testadas e descartadas: LLM por chunk (espalha o estado entre lotes, erra contagem e libera seções de outros campi) e regex sem supressões (sumário e tabelas-resumo viravam transições falsas). Resultado na base (migração de jul/2026, mesma função do pipeline aplicada via re-upsert com backup): 37 chunks liberados (10 do PDI, incluindo o Quadro 5.3 completo com as linhas de TADS e subtotais; 27 dos planos de ação 2016/2017/2012, seções "2.x Campus Canoas"). Os ids do PDI na base são numéricos legados ("8836"), não `url#i`.
...
Comportamento atual nesses casos: horários (dia e hora), disciplinas do professor (recall multi-curso, ex: TADS + Eng. Eletrônica), salas dos professores e número de servidores são respondidos corretamente, com o PDI e os docs institucionais fora do contexto. O número de servidores ganhou a moldura temporal por ano (ex: "para 2026 não encontrei; o mais recente é 2024: 70 docentes + 45 técnicos"). Em `total-vagas-campus`, o refino por chunk liberou o Quadro 5.3 do PDI (fonte Canoas de vagas planejadas) e as regras de prompt (não-somar + atribuição por ano) seguraram os runs de verificação: o agente responde com o total DECLARADO da fonte de Canoas ("404 vagas em 2023", com ressalva) ou lista por curso nomeando o ano, sem somar tabela; a re-coleta do eval para o caso ainda está pendente. Alguns PDFs do Drive (~18) mantêm chunks antigos por falha de rate limit no gdown, e o incremental semanal os retenta.
...
- **`campus_scope`: cap ativo + refino por chunk, monitorar.** Além da `CAMPUS_PENALTY=0.35` no rerank e do cap `CAMPUS_OUTRO_MAX=0`, o refino por chunk (`refinar_campus_scope`) libera do "outro" as seções de Canoas dentro dos docs institucionais (ver Retrieval); a base foi migrada em jul/2026 (37 chunks). Monitorar: se algum caso legitimamente institucional (reitor/PDI) ficar sem contexto por causa do cap, liberar alguns slots "outro" só quando o contexto ficar vazio.
...
Agente / conteúdo:
- Erros de comportamento abertos: `curso-inexistente` (em ~40% dos runs corrige a premissa mas PERGUNTA em vez de já buscar, derrubando a Fase 1; efeito colateral do reforço de correção) e `total-vagas-campus` (mitigado em jul/2026: refino por chunk liberou o Quadro 5.3 do PDI e os runs de verificação respondem com total declarado ou lista por curso com ano, sem somar; re-coleta do eval pendente para fechar com taxa). `inicio-aulas` foi FECHADO (o "03/08" era chunk-fantasma de calendário alucinado, removido; o `is_calendar_pdf` impede recriá-lo; hoje lidera com 27/07). A temperatura 0.7 gera flakiness de retrieval de ~1/15.
