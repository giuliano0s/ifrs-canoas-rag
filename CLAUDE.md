...
- **Grade de horários por VISÃO / pixelrag** (`structure_schedule_vision`, `parse_pdf`): a grade aSc TimeTables é tabela 2D que a extração de texto embaralha (célula transborda a linha, coluna funde) - foi a raiz do caso 1 (dia/hora sumia) e do recall do caso 2 (grade densa, professor enterrado). Detecção robusta por conteúdo ("aSc TimeTables") OU título "Horários_" OU estrutura (>=3 dias + horários + "criado"). Cada página é renderizada (`get_pixmap` dpi 200) e lida pelo gemini-2.5-flash (multimodal), que agrupa POR PROFESSOR. GATE anti-alucinação: horários/salas emitidos têm que existir no raw da página, senão a página é descartada (suspeita). `published_at` = ano do raw ("Horário criado:DD/MM/AAAA"). O chunker emite UM chunk por professor (`chunk_document(..., por_linha=True)`), não o splitter por tamanho, para o recall por professor não re-densificar. `schedule_source` (visao/visao_parcial/fallback_texto) marca grade quebrada.
...
Estado dos 5 casos (base reprocessada por REPARSE base-wide; validado por subagente): **caso 3** (servidores), **caso 5** (salas: "Prédio F" de Canoas, PDI fora do contexto), **caso 1** (horário: dia+hora na base via visão, todas as grades) e **caso 2** (disciplinas do professor: recall multi-curso - lista Est. Dados/Desenv. Web/IA reunindo TADS + Eng. Eletrônica) CONSERTADOS e testados ponta a ponta; **caso 4** (vagas) escopo corrigido (PDI fora) + instabilidade numérica tratada pela regra de não-somar-tabela. A **consistência temporal** (a ressalva de "dado de ano X" some por temperatura) é o item futuro MAIS IMPORTANTE (ver Próximos passos). Resíduo: ~18 PDFs do Drive falharam no gdown (rate limit) e mantêm chunks antigos; o incremental semanal os retenta.
...
Bateria completa (7 fases), n=5 em 355 execuções, carimbada por execução (modelo + hash do prompt + timestamp). A coleta pode misturar versões de prompt por ATUALIZAÇÃO MODULAR: ao mudar o prompt de um comportamento, re-coleta-se só os casos afetados (via `EVAL_IDS`), e os demais mantêm a coleta anterior, válida porque a mudança não os toca; o carimbo `prompt_versao` torna a mistura auditável, e o relatório declara quais casos estão em cada versão. Não é comparação entre versões, é medição modular. (CAVEAT: a versão `e2f8a4a2697b`, que carimba 250/355 execuções incl. as de segurança, é um prompt de working-tree e NÃO está em nenhum commit, então esses casos não são reproduzíveis a partir do git até uma re-coleta sob o prompt commitado; ver Pontos fracos.) Placar atual: Fase 1 decisão 99%, Fase 2 query 100%, Fase 3 retrieval doc 99%/span 99% (MRR 0.67), Fase 4 answerability 9/9 na base, Fase 5 fidelidade 100%/relevância 100%/correção 99%, Fase 6 comportamento 99%, Fase 7 citação 100%. Casos 100% limpos: 21/25.
