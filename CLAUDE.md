...
### Retrieval: escopo de campus, data e grade por visão (fixes da telemetria, jul/2026)

Refinamentos no retrieval que vieram de 5 casos reais da telemetria de produção (Langfuse), curados e virados casos de golden com `origem: "producao"`. A base foi reprocessada por REPARSE base-wide (todo o conteúdo não-escaneado: 2234 URLs / 6637 chunks reingeridos, grades por visão, `campus_scope` classificado), com backup completo antes (`backups/upstash/FULL-base-pre-reparse.jsonl`, 11.354 chunks, reversível).

- **Escopo de campus, no RERANK (não na query)**: a base é toda do Campus Canoas, mas contém docs INSTITUCIONAIS do IFRS (multi-campus, ex: o "PDI IFRS 2024-2028") que dominam a similaridade e enterram a resposta certa (a query "salas dos professores prédio" trazia 28/30 chunks do PDI). Duas peças: (a) `FETCH_K=60` (pool grande o bastante para o doc certo de Canoas, que fica além do top-30 quando o institucional domina a similaridade crua, entrar no pool); (b) penalidade no rerank `ALPHA*sim + (1-ALPHA)*date_score - CAMPUS_PENALTY`, com `CAMPUS_PENALTY=0.35` para chunks `campus_scope=="outro"` (ausência = neutro 0). Calibrada: a 0.20 o PDI ainda vazava 1 chunk no contexto de salas; a 0.30+ some (0.35 dá margem). Uma tentativa de ANCORAR a query (`f"{query} IFRS Campus Canoas"`) foi testada e REVERTIDA: resolvia o caso 5 (salas) mas inflava editais institucionais e enterrava a resposta em queries de professor (quebrava o caso 2). O `campus_scope` é atribuído no ingest por `classify_campus_scope` (ALTA PRECISÃO, para não expulsar doc de Canoas): tagueia "outro" só (a) fora do `/canoas/` E (b) com marcador institucional explícito (título "PDI"/"Plano de Desenvolvimento Institucional" ou >=4 campi do IFRS citados). NUNCA tagueia doc sob `/canoas/` (Canoas-específico mesmo citando a rede - senão o relatório CPA de Canoas virava "outro"). Hoje "outro" = o PDI (Drive, 269 chunks) + planos de ação IFRS institucionais (no `/site/`, não `/canoas/`).
- **Data do documento (`get_published_at`)**: lê o ano do PRÓPRIO documento primeiro. Ordem: nome do arquivo (`extract_date_from_filename`, só para arquivos reais `.pdf/.docx/...`, NUNCA o slug de uma página HTML, que traz ano de evento futuro) -> conteúdo (LLM, ~1000 chars: 500 do início + 500 do fim) -> pasta `/YYYY/MM/` da URL (data de UPLOAD) só como fallback. Antes a pasta de upload vinha primeiro, então um relatório de 2019 numa pasta `/2025/03/` virava `published_at=2025` e o rerank temporal o tratava como novo. `_date_score` agora tem piso 0 (doc antigo fica neutro, nunca negativo).
- **Grade de horários por VISÃO / pixelrag** (`structure_schedule_vision`, `parse_pdf`): a grade aSc TimeTables é tabela 2D que a extração de texto embaralha (célula transborda a linha, coluna funde) - foi a raiz do caso 1 (dia/hora sumia) e do recall do caso 2 (grade densa, professor enterrado). Detecção em `is_schedule_pdf` por 3 redes (qualquer uma dispara): assinatura "aSc TimeTables" no conteúdo, OU título "Horários_", OU rede estrutural (>=3 dias da semana por TOKEN via `\b(Seg|Ter|...|Segunda|Terça|...)\b`, NÃO substring - senão "Ter" casava "Território", "Qui" casava "Aqui" e disparava a visão à toa; mais o marcador "criado:" e >=3 horários HH:MM). Cada página é renderizada (`get_pixmap` dpi 200, teto `max_pages=30`) e lida pelo gemini-2.5-flash (multimodal), que agrupa POR PROFESSOR. GATE anti-alucinação: horários/salas emitidos têm que existir no raw da página, senão a página é descartada (suspeita). `published_at` = ano do raw ("Horário criado:DD/MM/AAAA"). O chunker emite UM chunk por professor (`chunk_document(..., por_linha=True)`), não o splitter por tamanho, para o recall por professor não re-densificar. **Os marcadores `is_schedule=True` e `schedule_source` (visao/visao_parcial/fallback_texto) são PERSISTIDOS no metadata do Upstash** (só nos chunks de grade; ausentes nos demais para não inchar o metadata), tornando as grades auditáveis/filtráveis na base e sinalizando grade incompleta ou degradada - o que o texto do chunk sozinho não revela (uma grade parcial parece completa, só com menos professores). Truncagem (>`max_pages`) ou página pulada/descartada marca `visao_parcial`, então a incompletude fica VISÍVEL, não silenciosa. O conjunto na base = exatamente as grades linkadas na página oficial `ensino/horarios-das-aulas/` (10 do semestre 2026/2); a grade stale 2026/1, já deslinkada e num formato antigo degradado (1 chunk), foi removida (backup em `backups/upstash/grades-pre-fix.jsonl`).
- **Prompt**: regra de ENUMERAÇÃO (pergunta que pede TODAS as X de Y deve buscar, não responder de memória, e admitir incompletude) e regra de NÃO-SOMAR-TABELA (não cravar um total somando células; citar total declarado ou dar por item - corrige a instabilidade numérica do caso 4).

Estado dos 5 casos (spot-check ao vivo n=3 nesta sessão; base reprocessada): **casos 1** (horário: dia+hora na base via visão, todas as grades), **2** (disciplinas do professor: recall multi-curso reunindo TADS + Eng. Eletrônica) e **5** (salas: "Prédio F" de Canoas, PDI fora do contexto) CONSERTADOS e sólidos (3/3). **Casos 3** (servidores) e **4** (vagas): escopo/retrieval corrigidos (o doc de 2019 e o PDI ficaram fora do contexto), MAS com resíduo ABERTO - a ressalva temporal some em parte dos runs e a soma numérica ainda oscila (servidores às vezes mistura anos -> 112; vagas às vezes crava um total somando editais heterogêneos -> 135; a regra de não-somar-tabela reduziu, não zerou). A **consistência temporal** (a ressalva de "dado de ano X" some por temperatura) é o item futuro MAIS IMPORTANTE e cobre o resíduo de 3 e 4 (ver Próximos passos). Resíduo de ingestão: ~18 PDFs do Drive falharam no gdown (rate limit) e mantêm chunks antigos; o incremental semanal os retenta.
...
**Prioridade máxima (fixes da telemetria de produção, jul/2026; ver "Retrieval: escopo..."):**
- **[O MAIS IMPORTANTE, ABERTO] Consistência temporal.** A ressalva "dado de ANO, pode estar desatualizado" (regra que JÁ existe no prompt) some em parte dos runs por temperatura 0.7. É o ponto mais crítico a fechar: um número/data antigo apresentado como vigente engana o estudante. Defesas a testar e medir no eval: temperatura menor na geração; um guard de saída que FORCE a ressalva quando a fonte citada tem ano < ano atual; ou reforço de prompt. Atinge os casos 3 e 4. Inclui também dar granularidade de SEMESTRE à data das grades (o título traz "2026_2"): hoje `_date_score` compara só por ANO, então uma grade de 2026/1 empataria com as de 2026/2 no rerank; é o fix sistêmico que blinda contra uma grade de semestre anterior voltar à base (re-crawl) e competir com a vigente (hoje mitigado só por a grade stale estar deslinkada da página oficial).
- **Grade de horários por visão (pixelrag): FEITO** (aplicado base-wide). Detecção por conteúdo/título/estrutura, extração por visão (render + gemini-2.5-flash) agrupando POR PROFESSOR, gate anti-alucinação (token no raw), data determinística do raw, chunk-por-professor. Resolveu o caso 1 (dia+hora na base) e o recall do caso 2 (multi-curso). Endurecimento posterior (jul/2026, validado por subagente): os marcadores `is_schedule`/`schedule_source` passaram a ser PERSISTIDOS no metadata (antes eram calculados e descartados no upsert), tornando as grades auditáveis e a incompletude visível; `max_pages` 14->30 com truncagem virando `visao_parcial` (fim da truncagem silenciosa); detecção estrutural por TOKEN (não substring); as 10 grades atuais reingeridas via visão (366 chunks, todas `visao`, `published_at=2026`, backup em `backups/upstash/grades-pre-fix.jsonl`); e a grade stale 2026/1 (deslinkada, degradada) removida - o conjunto na base = as 10 grades da página oficial `ensino/horarios-das-aulas/`. Residual: durabilidade da limpeza da stale depende de granularidade de semestre no rerank (ver item de consistência temporal). Follow-up: estender a visão aos ~1359 escaneados (hoje pulados) - escopo novo, custo maior.
- **Número de vagas (caso 4): FEITO** (regra de não-somar-tabela no prompt; o agente cita total declarado ou dá por item, em vez de cravar total somado instável).
- **Classificador de `campus_scope`: FEITO** (`classify_campus_scope`, aplicado no REPARSE; alta precisão: nunca tagueia `/canoas/`, marca "outro" só institucional fora do campus; penalidade calibrada a 0.35). ABERTO/monitorar: -0.35 pode ainda ser insuficiente numa query dominada pelo institucional (o agente escapa na prática reduzindo a query a keywords); considerar cap de slots "outro" se reaparecer.

Roadmap mais amplo:
1. Endurecer a produção para uso público: FEITO os caps de tamanho (query + histórico por chars) + teto global de volume no dia (`GLOBAL_DAILY_MAX`), o rate limit com IP real do Vercel (não o `X-Forwarded-For` spoofável), o fim do fonte servido (`/app.py` -> 404 por whitelist) e o fail-closed controlado no caminho do Redis (503 limpo em vez de 500). ABERTO: um guard de saída (2ª barreira: um juiz validador do OUTPUT antes de entregar a resposta, que barre vazamento do prompt ou saída fora do papel mesmo quando o input passou; hoje isso só é checado pelo juiz da Fase 6 no eval, NÃO em produção) e aviso/redação de PII na telemetria (LGPD, adiado na fase de testes). Fora de escopo do app: injeção indireta (governança de conteúdo do campus) e DDoS/abuso distribuído (borda do Vercel).
...
3. Fechar os erros de comportamento abertos via prompt, medindo no eval: `curso-inexistente` (flagrar a inexistência ANTES de redirecionar ao TADS, sem substituição silenciosa) e `inicio-aulas` (não liderar com data passada). `mensalidade` já passou a 15/15 (regra de gratuidade), falta só re-confirmar sob o prompt commitado. Junto: A/B de temperatura (0.2 vs 0.7) contra a flakiness de 1-em-15; e reportar o IC POR CASO no placar (ver Pontos fracos).
...
Eval / metodologia:
- Juiz não calibrado contra rótulo humano: as Fases 5/6 são sinal, não régua. Discriminação baixa na coleta atual (fidelidade 178/178, relevância 355/355, só 4 "false" em ~1006 julgamentos), o que reforça tratar 5/6 como sinal.
...
- IC do placar é agregado (pooled) sobre execuções correlacionadas (5 runs × paraphrases de 25 intenções), o que ESTREITA o IC artificialmente; a leitura honesta é o IC POR CASO (o relatório já traz um por caso). Mais amostra esclarece a realidade, não piora; o ajuste é só reportar/agregar por caso, não no bolo de 355.
- Baseline mistura versões de prompt: 250/355 execuções (incl. os 6 casos de segurança) rodaram sob `e2f8a4a2697b`, um prompt de working-tree que NÃO está em nenhum commit; o commitado (`fd803de40dc3`) só tem 105 exec / 7 casos. O e2f8 é quase idêntico ao commitado (diferenças pontuais por caso), mas por não estar no git a prova desses casos não é reproduzível/auditável. Re-coletar 100% sob o prompt commitado (com n alto em segurança) é pendência: fecha a auditabilidade e mede a segurança no prompt que vai pro ar.
...
Agente / conteúdo:
- Erros de comportamento abertos (bugs reais de baixa frequência, ~1/15, NÃO ruído): `curso-inexistente` (substituição silenciosa) e `inicio-aulas` (lidera com data passada). `mensalidade` já está em 15/15 na coleta (regra de gratuidade no prompt), pendente só de re-confirmar sob o prompt commitado. Temperatura 0.7 gera a flakiness de retrieval de 1-em-15.
...
Golden set atual: **30 casos** (25 originais + 5 da telemetria de produção, marcados `origem: "producao"` com o `trace_id`, ainda NÃO coletados/validados). Os 25 originais somam 71 inputs. Distribuição por ação dos 25 originais: `buscar` 9, `perguntar` 5, `recusar` 5, `corrigir_e_buscar` 3, `responder_direto` 3. Os 5 de produção (numero-servidores, salas-professores-predio, total-vagas-campus, disciplinas-professor, horario-aulas-turma) são alvos de regressão dos fixes de telemetria; ainda não passaram pela coleta FORMAL do eval, mas foram verificados por spot-check ao vivo (n=3) nesta sessão: `disciplinas-professor` agora PASSA o retrieval (a grade foi reestruturada por visão) e o resíduo aberto está só em servidores/vagas (ver "Estado dos 5 casos").
...
**[PLACAR DESATUALIZADO - re-coleta pendente]** Os números abaixo são ANTERIORES às mudanças desta sessão (o prompt ganhou as regras de ENUMERAÇÃO e NÃO-SOMAR-TABELA; a base foi reprocessada por REPARSE base-wide + re-ingest das grades), então NÃO refletem o estado vigente; servem de referência histórica até uma re-coleta sob o prompt e a base atuais, de preferência com n alto nos casos de baixa frequência (ex: inicio-aulas, curso-inexistente, segurança). Bateria completa (7 fases), n=5 em 355 execuções, carimbada por execução (modelo + hash do prompt + timestamp). A coleta pode misturar versões de prompt por ATUALIZAÇÃO MODULAR: ao mudar o prompt de um comportamento, re-coleta-se só os casos afetados (via `EVAL_IDS`), e os demais mantêm a coleta anterior, válida porque a mudança não os toca; o carimbo `prompt_versao` torna a mistura auditável, e o relatório declara quais casos estão em cada versão. Não é comparação entre versões, é medição modular. (CAVEAT: a versão `e2f8a4a2697b`, que carimba 250/355 execuções incl. as de segurança, é um prompt de working-tree e NÃO está em nenhum commit, então esses casos não são reproduzíveis a partir do git até uma re-coleta sob o prompt commitado; ver Pontos fracos.) Placar atual: Fase 1 decisão 99%, Fase 2 query 100%, Fase 3 retrieval doc 99%/span 99% (MRR 0.67), Fase 4 answerability 9/9 na base, Fase 5 fidelidade 100%/relevância 100%/correção 99%, Fase 6 comportamento 99%, Fase 7 citação 100%. Casos 100% limpos: 21/25.
...
- Hardening de segurança no prompt (item 0 jailbreak/injeção + regra 3 fora-de-escopo): `jailbreak-complexo` deixou de vazar o prompt e `fora-escopo-basico` parou de responder conhecimento geral, sem over-refuse. CAVEAT DE AMOSTRA: na coleta commitada a segurança está em n=5 por input (jailbreak-complexo 5/5, fora-escopo-basico 15/15), IC largo (Wilson de 5/5 = [57-100%]); a re-medição a n alto (20-60) que motivou o conserto NÃO está na coleta commitada. Re-coletar segurança com n alto é pendência (ver Pontos fracos).
...
Erros abertos. Com n=5 (ou 15 nos casos com paraphrases) NÃO dá para afirmar que uma falha de 1 execução é ruído aleatório e não bug sistemático de baixa frequência; até re-medir com n alto, os de comportamento/correção abaixo são tratados como BUGS REAIS a fechar, não como ruído:
- `curso-inexistente` (bug de comportamento): em ~1/15 faz substituição silenciosa (responde o coordenador do TADS sem antes sinalizar que Engenharia de Software não é ofertada). Baixa frequência, mas é defeito real, não ruído. Ponto à parte na Fase 1: alguns runs corrigem a premissa e OFERECEM buscar (`nao_buscar`), ação aceitável que a Fase 6 aprova mas a Fase 1 (decisão buscar/não-buscar) conta como divergência (daí ~11/15); candidato a `acao_esperada` em lista. (O "inventa 2236h" do diagnóstico antigo era artefato da truncagem do juiz, já corrigida: o número estava no contexto.)
- `inicio-aulas` (bug de correção/temporal): em ~1/15 lidera com data passada (23/02/2026) em vez da próxima (27/07/2026). Baixa frequência, defeito real.
- Flakiness de retrieval (`biblioteca`, `complementares`): doc 14/15. Esse sim é ruído de SORTEIO do top-15 (o conteúdo existe na base, entra/não conforme o draw), instabilidade de amostragem do retrieval, não bug de lógica; some ao subir n ou estabilizar o rerank.
